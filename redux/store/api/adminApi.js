...
export const adminApi = createApi({
  ...
  tagTypes: ["AdminDashboard", "AdminUsers", "AdminFilms", "AdminPayments", "AdminDistro", "AdminSubscribers"],
  endpoints: (builder) => ({
    // Dashboard
    ...
    getAdminUsers: builder.query({
      query: ({ user_id, search } = {}) => {
        ...
        return `/manage-users${params.toString() ? `?${params.toString()}` : ""}`;
      },
    ...
    getAdminSubscribers: builder.query({
      query: ({ search } = {}) => {
        const params = new URLSearchParams();
        if (search) params.append("search", search);
        return `/subscribers${params.toString() ? `?${params.toString()}` : ""}`;
      },
