...
export default function TrendingNow() {
  const { data: trendingResponse, isLoading, error } = useGetTrendingFilmsQuery();
      ...
      <div className="container my-10 md:my-16" id="trending-movie">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-5">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="bg-secondary animate-pulse rounded-md h-80" />
          ))}
        </div>
      </div>
    );
  ...
  const filmsToShow = trendingFilms.length > 0 ? trendingFilms : moviesData.slice(0, 8);
        ...
        {filmsToShow.map((movie) => (
          <MovieCard 
            ...
            movie={{
              ...
              duration: movie.duration || 120, // fallback duration
            }} 
          />
        ))}
      </div>
