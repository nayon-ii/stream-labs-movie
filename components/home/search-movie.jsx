...
export default function SearchMovie({ className }) {
  ...
  const { data: searchResponse, isLoading } = useSearchFilmsQuery(debouncedSearchTerm, {
        ...
        <div className="absolute top-full w-full bg-background/20 backdrop-blur-md border rounded-lg mt-2 left-0 max-h-96 overflow-y-auto custom-scrollbar p-3">
          ...
          {movies.length > 0 ? (
            <div className="space-y-3 mt-3">
              {movies.map((movie) => (
                <Link key={movie.id} href={`/film/${movie.id}`} className="flex gap-3 hover:bg-secondary/50 p-2 rounded-md">
                  ...
                  <div className="flex flex-col gap-1 min-w-0">
                    <p className="text-lg font-medium truncate">{movie.title}</p>
