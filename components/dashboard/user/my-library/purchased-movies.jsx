...
export default function PurchasedMovies() {
  ...
  const { data: libraryResponse, isLoading, error } = useGetMyLibraryQuery({
    access_type: filter.selectedOption === "All" ? undefined : filter.selectedOption,
      ...
      <div className="my-5">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="bg-secondary animate-pulse rounded-md h-80" />
          ))}
        </div>
      </div>
    );
